...
- ~~⏳ NEXT (ครึ่งหลังของ C3)~~ → ✅ ทำเสร็จ 2026-07-07 (ดู entry ถัดไป)

### 2026-07-07 (Tier C — C3 ครึ่งหลัง: client เรียก 9 RPCs, browser-verified) — **MIGRATION จบ**
ปิดครึ่ง client ของ C3 — 9 money actions เลิก dual-write หลายตาราง เปลี่ยนเป็น optimistic set() + 1 RPC atomic:
- **`lib/row-mappers.ts` (ใหม่):** hoist mappers `rowTo*`/`*ToRow` ทั้งหมด (28 ตัว) ออกจาก AppShell → แชร์ระหว่าง AppShell (channels/reconcile) กับ `callRpc` ใน store (AppShell เหลือ import อย่างเดียว, -380 บรรทัด)
- **`lib/store.ts` — โครง callRpc:** `callRpc(name, params, repairSpec, {absentNull})` ยิงหลัง optimistic set() (background ไม่ await — UI ตอบทันที):
  - **success** → `applyRpcRows()` apply แถว authoritative ที่ RPC คืน (jsonb ทุกแถวที่เขียน) ทับ optimistic ผ่าน `upsertSlice` (จำเป็นเพราะ echo-suppression ทำให้แท็บต้นทางไม่ได้รับ realtime ของตัวเอง; ค่า derive จาก live rows อาจต่างจากที่ client คิด) — ครอบ `applyRemoteState` กันเขียน blob กลับ
  - **error** (`CODE|ข้อความไทย` → โชว์เฉพาะไทย + "การเปลี่ยนแปลงถูกย้อนกลับ") → `repairFromTables(spec)` refetch แถวที่ optimistic แตะจากตารางจริง (id ไม่พบ = แถวผีที่ optimistic สร้าง → ถอดออกจาก state = rollback จริง) + toast
  - **absentNull:** แถวที่ client สร้าง optimistic แต่ RPC (จาก live rows) ตัดสินใจไม่สร้าง (HK ข้ามเพราะห้องมีซ่อมค้าง / corp charge เครดิตไม่พอ / ledger ไม่ต้องคืน) → key ใน result เป็น null = ถอด id นั้นออก
- **wire 9 actions:** create (advisory lock กัน phantom overbooking) / checkout (RPC 6 ตาราง) / cancel / move / recordPayment / fulfillAddOn / cancelAddOn / extend (**ส่ง `p_old_check_out` CAS**) / adjustEarlyCheckout — id/timestamp ทุกตัว **gen ก่อน set()** ใช้ร่วม optimistic+RPC (แถว DB == optimistic ใน happy path); ตัด closure-Fx dual-writes + `invoiceRow`/`reportInvoiceError` ทิ้ง. **residual คงไว้ตามแผน:** check-in branch + transition อื่น (pushBooking/pushRooms), `updateBooking`/`requestAddOn` (single insert/patch), `updateTaskStatus`
- 🐞 **บั๊กเจอตอน verify (แก้แล้วทั้งไฟล์ 020 + live DB):** `create_booking_with_conflict_check` insert `p_booking->>'created_at'` (text) ลง `bookings.created_at` = **คอลัมน์ timestamptz ตัวเดียวของตาราง** (ที่เหลือเป็น text หมด — เช็คแล้วทุกคอลัมน์ timestamp ที่ RPC อื่นเขียน) → text ไม่ implicit-cast = RPC พังตอนรันจริง (เทสต์ BEGIN/ROLLBACK รอบก่อน insert ตรงไม่ผ่าน RPC create เลยไม่เจอ) → cast `::timestamptz`. **ข้อดี: บั๊กนี้พิสูจน์เส้นทาง repair สด ๆ** — booking ผีถูกถอดจาก UI + toast ขึ้นถูกต้อง
- **✅ browser-verify ผ่าน (CDP→Windows Chrome, dev server + prod DB):**
  - **happy path เต็ม lifecycle:** create advance VERIFY-C3 (paid=0) → จ่ายบางส่วน 100 → extend +1 คืน (CAS ผ่าน, total 1000→1500) → cancel (refund -100, paid=0, invoice=0, HK=0, **ห้องไม่ถูกแตะ** — rA2 cleaning เดิมจาก 07-01) ทุกแถว stamped
  - **phantom-create race (2 แท็บ isolated contexts ยิง submit พร้อมกัน ห้อง/วันเดียวกัน):** แท็บแพ้ได้ ROOM_CONFLICT → toast + repair ถอด booking ผีออกจาก list; ตาราง DB มี **1 แถวเท่านั้น**; สองแท็บ converge เห็นแถวเดียวกัน (แถวผู้ชนะมาทาง realtime)
...
### 2026-07-13 (Supabase Auth — email/password จริง + authenticated tables) — branch `feat/supabase-auth` (stack บน phase4)
เลิก mock login (เทียบ bcrypt ใน store.users + session localStorage) → **Supabase Auth จริง**. บริบทเปลี่ยนหลัง retire blob: residual risk ไม่ใช่ "anon อ่าน blob" อีก (blob=`{}`) แต่เป็น "anon อ่านตาราง relational". **Decisions (user):** email/password ล้วน (ตัด demo buttons) · authenticated ทุกตาราง **ไม่ทำ per-role DB RLS** (role gate ที่ client พอ — small trusted team) · seed 6 บัญชีคงที่ (ตัด create-login ใน UI; สร้าง/ลบต้อง service_role ห้ามอยู่ client)
- **ลำดับปลอดภัย (DB เดียว = dev/prod ร่วมกัน; additive ก่อน destructive ทีหลัง):**
  - **ขั้น A (022, apply แล้ว):** `users.auth_id uuid` + seed 6 auth.users (email `<username>@pruksatara.local` + known pw, email-confirmed) via SQL. 🐞 gotcha: token cols (`confirmation_token`/`recovery_token`/`email_change*`/`phone_change*`/`reauthentication_token`) ต้อง `''` ไม่ใช่ NULL — ไม่งั้น GoTrue 500 "Database error querying schema" (scan เป็น Go string). `identities.email`+`users.confirmed_at` เป็น GENERATED ห้าม insert
  - **ขั้น B (023, apply แล้ว):** `ALTER POLICY ... TO anon, authenticated` ทุก policy (public 18 ตาราง + storage receipts) — additive, prod เดิม (anon) ยังทำงาน + authenticated JWT อ่าน/เขียนได้
  - **ขั้น C (โค้ด, PR นี้):** `auth-store.ts` เขียนใหม่ (supabase.auth: `init` getSession+onAuthStateChange / `login` signInWithPassword→resolveSession(users.auth_id→staff→permissions) / `logout` signOut; คง `SessionUser` shape เดิม → 15 consumer ไม่แตะ) · **AppShell boot inversion** (hydration useEffect gate บน `authUserId` = login ก่อน→hydrate; render gate เรียงใหม่ auth→user→data) · login page email/password ตัด demo panel + `_hasHydrated` gate · ChangePassword → `supabase.auth.updateUser` (re-auth current pw) · ตัด AccountsManager (create/edit/delete/reveal-pw บัญชี) ออกจาก /staff (staff record+permission ยังจัดการได้)
  - **ขั้น E (024, ⚠️ ยังไม่รัน — user-gated หลัง deploy):** `ALTER POLICY ... TO authenticated` (ถอด anon) = hardening จริง. **ต้องรันหลัง** merge+Vercel deploy+login prod ผ่าน (ไม่งั้น prod anon เดิมพัง)
- **✅ browser-verify ผ่าน (CDP→Chrome, dev + prod DB, authenticated policies):** unauth→redirect /login; login admin/reception/nida (email/pw) ✅; demo panel ตัดแล้ว ✅; **session persist หลัง reload** (getSession restore) ✅; role menu ถูก (nida housekeeper ไม่มี staff menu) ✅; wrong pw → "อีเมลหรือรหัสผ่านไม่ถูกต้อง" ✅; logout→/login + session cleared ✅; last_login dual-write ✅; **0 console errors**; tsc 0 / build 22
- ⏳ **NEXT:** merge phase4 PR #23 → rebase auth PR onto main → merge → **Vercel deploy** → login prod ผ่าน → **รัน 024 (drop anon)** = ปิด residual สนิท. แล้วไป Night Audit

### 2026-07-27 (เตรียมใช้งานจริง #1 — ผังห้องแก้เองได้ + จัดการบัญชีล็อกอิน + เลิกปุ่มสำรอง/รีเซ็ตที่พัง)
งานชุด "go-live" (ไม่ใช่ฟีเจอร์ใหม่ แต่คือสิ่งที่ขวางไม่ให้เอาไปใช้จริง) — branch `main` local
- **⚠️ Supabase project ถูก pause อัตโนมัติ** (free tier หยุดโปรเจกต์ที่ไม่มีทราฟฟิก ~7 วัน; เจอตอน query แล้ว connection timeout, status=INACTIVE) → `restore_project` ปลุกกลับแล้ว. **ใช้งานจริงต้องอัป Pro** (~$25/เดือน) ไม่งั้นระบบดับเองตอนเงียบ + free tier ไม่มี daily backup
- **Room CRUD (ใหม่):** เดิม rooms เป็นชุดคงที่จาก seed (แก้ผังห้อง/ราคาต้องยิง SQL) — เพิ่ม `addRoom`/`updateRoom`/`deleteRoom` ใน store (สิทธิ์ `canManageRooms`, validate เลขห้องซ้ำ/ราคา>0/ชั้น>0, audit ทุก action) + UI ใน `/rooms` แท็บ "รายการห้อง" (ปุ่มเพิ่มห้อง + คอลัมน์จัดการ + dialog). **ลบ = soft-delete** (ประวัติจอง/HK/ซ่อม อ้าง room_id อยู่) + บล็อกเมื่อห้อง `occupied` หรือมีการจองที่ยังไม่จบ. `RoomInput` type ใหม่ใน types (ตัด status/pointers ที่ lifecycle คุม)
- **✅ browser-verify ผ่าน (CDP→Chrome, dev + prod DB):** เพิ่มห้อง VERIFY-Z9 (40→41 แถว) → insert ถึงตารางครบทุกฟิลด์ + stamped · แก้ราคา 1,234→999 → UI + คอลัมน์ `price_per_night` เปลี่ยนตาม · เลขห้องซ้ำ "A1" → ถูกปฏิเสธพร้อม toast · ลบ → soft-delete (deleted_at set) + หายจาก UI (41→40) · audit 3 แถวถูกต้อง · 0 console errors. cleanup: hard-delete แถวทดสอบ + audit ออกแล้ว (rooms = 40 live)
- **จัดการบัญชีล็อกอิน (ใหม่ — ปิดช่องโหว่ใช้งานจริง):** หลังย้ายไป Supabase Auth บัญชีถูกล็อกไว้ 6 บัญชี seed (พนักงานเข้าใหม่/ลืมรหัส = ต้องยิง SQL). เพิ่ม **`app/api/accounts/route.ts`** (route handler แรกของโปรเจกต์, runtime nodejs) รับ 3 คำสั่ง `create`/`reset-password`/`delete` — ตรวจ token ผู้เรียก → users→staff→`canManageStaff` ฝั่ง server ก่อนเรียก `auth.admin.*` (service_role ห้ามอยู่ client) + เขียน audit ฝั่ง server. UI = `components/AccountsManager.tsx` ในหน้า `/staff` (list บัญชี + เพิ่ม + ตั้งรหัสใหม่ + ลบ, กันลบบัญชีตัวเอง, พนักงาน 1 คน 1 บัญชี)
  - **⏳ ต้องตั้ง env `SUPABASE_SERVICE_ROLE_KEY`** (ทั้ง `.env.local` และ Vercel Production) ไม่งั้น API คืน 503 พร้อมข้อความบอกเหตุ — verify แล้วว่าหน้า /staff เรนเดอร์ปกติ (6 บัญชี) และปุ่ม "เพิ่มบัญชี" ถูก disable เมื่อพนักงานทุกคนมีบัญชีแล้ว
...
3. **ความปลอดภัย: RLS** — 🟢 **2026-07-13 Supabase Auth จริงแล้ว** (branch `feat/supabase-auth`): retire blob ทำให้ blob=`{}` (ไม่มีข้อมูลให้ anon ดึงผ่าน blob แล้ว) + ย้าย mock login → Supabase Auth email/password + ทุกตาราง `TO authenticated` (ขั้น B/023 apply แล้ว). **เหลือขั้นเดียว = รัน `024` (drop anon) หลัง deploy** = ปิด residual "anon key อ่านตาราง" สนิท. (เลือก **ไม่ทำ per-role DB RLS** — role gate ที่ client พอสำหรับทีมเล็ก trusted). ก่อนหน้า: 🔶 2026-06-18 interim hardening (migration 012) ปิด anon บน orphan/PII + audit_logs insert-only + store permission guards (ดู §5 entry 2026-06-18). P3 ค้าง: session expiry/revoke ตอนนี้ Supabase Auth จัดการให้ (JWT + refresh); `receipts` bucket = authenticated แล้ว (023)
